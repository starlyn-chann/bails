    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList, AI = false }) => {
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isGroupOrStatus && !isRetryResend) {
                ...
                const groupAddressingMode = groupData?.addressingMode || 'lid';
                const participantJidForMode = (participant) => {
                    if (groupAddressingMode === 'lid') {
                        return participant.lid || (isLidUser(participant.id) ? participant.id : undefined);
                    }
                    return participant.phoneNumber || (isPnUser(participant.id) ? participant.id : undefined);
                };
                const participantsList = groupData
                    ? groupData.participants.map(participantJidForMode).filter(Boolean)
                    : [];
                ...
                const groupAddressingModeForEncryption = additionalAttributes?.['addressing_mode'] || groupAddressingMode;
                const groupSenderIdentity = groupAddressingModeForEncryption === 'lid' && meLid ? meLid : meId;
